            ...
            <tbody>
              <?php foreach ($brands as $brand): ?>
                <tr role="row" class="odd">
                  ...
                  <td class="sorting_1">{{$brand->en_name}}</td>
                  <td>
                      <a href="{{route('brand.edit', ['brand'=>$brand->id])}}" type="button" class="btn btn-block btn-warning">ویرایش</a>
                  </td>
                  <td>


                    <form class="" action="{{route('brand.destroy', ['brand'=>$brand->id])}}" method="post">
                      {{csrf_field()}}
                      {{method_field('delete')}}
