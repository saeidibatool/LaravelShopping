...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">لیست فیلترهای محصولات</h3>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
           ...
           <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
            <thead>
              <tr role="row">
                ...
                <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 269.483px;" aria-label="دسته‌بندی: activate to sort column ascending">دسته‌بندی</th>
                <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 73.35px;" aria-label="ویرایش: activate to sort column ascending">ویرایش</th>
            ...
            <tbody>
              <?php foreach ($filters as $filter): ?>
                <tr role="row" class="odd">
                  <td class="sorting_1">{{$filter->name}}</td>
                  <td>
                    {{$filter->en_name}}
                  </td>
                  <td>
                    {{$filter->category->title_fa}}
                  </td>
                  <td>
                      <a href="{{route('filter.edit', ['filter'=>$filter->id])}}" type="button" class="btn btn-block btn-warning">ویرایش</a>
                  </td>
                  <td>


                    <form class="" action="{{route('filter.destroy', ['filter'=>$filter->id])}}" method="post">
                      {{csrf_field()}}
                      {{method_field('delete')}}
