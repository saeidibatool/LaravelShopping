      ...
      <form role="form" method="post" enctype="multipart/form-data" action="{{route('role.update', ['role'=>$role->id])}}">
        ...
        <div class="box-body">
          <div class="form-group">
            ...
            <input name="name" type="text" class="form-control" id="exampleInputEmail1" value="{{$role->name}}">
          </div>
          <div class="form-group">
            <label for="exampleInputPassword1">عنوان</label>
            <input name="title" type="text" class="form-control" id="exampleInputPassword1" value="{{$role->title}}">
          </div>
            ...
            <label for="exampleInputPassword1">دسترسی‌ها</label>
             <select class="form-control" name="permission_id[]" multiple>
               <?php foreach ($permissions as $permission): ?>
                 <?php if ($role_permissions->contains('name',$permission->name)): ?>
